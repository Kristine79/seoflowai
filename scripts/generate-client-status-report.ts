...
const OVERRIDES: Record<string, Override> = {
  ...
  Crunchbase: { status: "Аккаунт создан", result: "Требуется заполнение профиля", next: "Заполнить профиль компании", comment: "Регистрация завершена" },
  Medium: { status: "Аккаунт создан", result: "Требуется публикация", next: "Создать страницу компании", comment: "Аккаунт существует" },
  ...
  "Bark.com": { status: "Требуется действие", result: "Ожидает действия", next: "Заполнить форму вручную", comment: "Многошаговая форма зависает в автоматизации" },
  Semfirms: { status: "Требуется действие", result: "Ожидает действия", next: "Зарегистрироваться и добавить компанию", comment: "Требуется аккаунт" },
