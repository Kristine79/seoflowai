...
const RESULTS = [
  ...
  // NEEDS_MANUAL - real forms but blocked
  ...
  // NEEDS_MANUAL - login/registration required
  ...
  // NOT_APPLICABLE - не каталоги размещения
...
const rows = RESULTS.map(r => {
  ...
  const evidence = r.evidence.slice(0, 40);
  return `${r.platform.padEnd(25)} | ${url.padEnd(42)} | ${type.padEnd(15)} | ${reason.padEnd(42)} | ${evidence}`;
});
...
console.log(`\nTOTAL: ${RESULTS.length} directories`);