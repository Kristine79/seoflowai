...
- **Email verification** — confirmation link/code required; handled manually unless valid IMAP credentials are configured (see below).
...
- **Moderation / manual review** — partner applications, editorial review, manual approval.
- **Unusual or unfinished steps** — e.g., a submit that does not complete within the window, unusual forms the engine does not recognize.
...
3. The form is extracted and fields are auto-filled (rule + AI mapping). An **email policy** applies:
   - registration/login/verification fields get the registration email;
   - business/company/contact fields get the public company email.
...
Automated email verification exists in code (`src/lib/automation/email-verifier.ts`, using `ImapFlow`), and `human-submit.ts` has a `--register` mode that attempts it. **It only works when valid IMAP credentials are configured** (`.env`). Without them, email-verification steps are handled manually where a platform requires them.
...
- **No CAPTCHA bypass** — challenges are always surfaced to the human.
- **No anti-CAPTCHA services** — the harness never delegates to captcha-solving services.
- **No blind retry** — protected sites are not requested repeatedly; a block is recorded with the reason and the next platform is processed (see `AGENTS.md`).
- **No fake success** — generic "success" text is never treated as proof of submission; statuses are set from evidence.
- **No automatic payment** — payment information is never entered automatically.
...
- When a site blocks access, record `BLOCKED`/`FAILED` with the external-block reason and move to the next platform.
