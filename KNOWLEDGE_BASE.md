...
>
> Nothing in this document exists in the codebase yet. It is a proposed direction for future architecture. This section exists specifically so the roadmap is documented separately from current behavior and is **not** confused with the current engine (see `DIRECTORY_ENGINE.md`).

## Vision

The long-term direction is a **Directory Knowledge Base + Embeddings + RAG + Flow Classifier + Generic Adapters** system that lets SEOFlow learn from every directory it touches:
...
- **RAG does not replace browser automation.** Automation (Playwright harness, form filling, submission) stays as-is; RAG only acts as a **memory/retrieval layer** that improves flow selection and guidance.
